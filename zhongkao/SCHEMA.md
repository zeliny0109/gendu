...
## 5. 题型攻略 `data/<科目>/types.json`

```json
{
  "strategy": "整卷策略（Markdown）：时间分配、做题顺序、检查方法、各大题分值一览",
  "types": [
    { "id": "shuxue.t.proof18", "name": "第 18 题 全等证明", "pos": "解答题第 18 题", "score": 8, "minutes": 6,
      "qs": ["shuxue-2019-18", "shuxue-2021-18", "…"],
      "md": "## 这道题长什么样\n…\n\n## 解题步骤\n1. …\n\n## 答题规范（得分点）\n- …\n\n## 常见失分\n- …\n\n## 真题示范\n### 2026 年第 18 题 [[q:shuxue-2026-18]]\n…" }
  ]
}
```
- 一张卡对应试卷上一个**固定题位或固定题型**（如“数学第 18 题”“英语完形填空”“化学第 17 题实验探究”“道法判断说理”），每科 6–10 张，按试卷顺序排列。
- `qs`：历年真题里属于这个题型的全部题目 id（按小问拆的题，列出所有小问），供“按题型连着练”。
- `md` 固定五节：**这道题长什么样**（历年题位、分值、设问形式的变化，依据 papers）→ **解题步骤**（可操作的 3–6 步）→ **答题规范（得分点）**（从官方参考答案/评分要点里归纳，不编造）→ **常见失分** → **真题示范**（2–3 道不同年份的题，演示同一套步骤怎么用，结论与官方答案一致）。
- 600–1500 字；写给九年级学生，短句、可操作。
- `strategy` 写整卷：各大题分值、建议用时、做题顺序、最后检查什么。

检查：`python3 tools/zk_types_check.py <科目>`
